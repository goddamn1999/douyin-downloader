...
<body>
    <div class="container">
        ...
        <div class="input-group">
            <input type="text" id="url" placeholder="https://v.douyin.com/xxxxx 或 https://www.douyin.com/video/xxxxx">
    ...
    <script>
        async function parse() {
            ...
            if (!url) {
                ...
                result.innerHTML = '<p class="error">请输入链接</p>';
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    result.innerHTML = `
                        <h3 class="success">✅ 解析成功</h3>
                        <p><strong>类型:</strong> ${data.data.type}</p>
                        <p><strong>ID:</strong> ${data.data.id}</p>
                        <h4>下载命令：</h4>
                        <div class="code-block">python run.py -u "${data.data.resolved_url}"</div>
                        <p style="margin-top:10px;color:#666;font-size:14px;">💡 请在本地安装后运行此命令下载视频</p>
                    `;
                } else {
                    result.innerHTML = '<p class="error">❌ ' + (data.message || '解析失败') + '</p>';
                }
            } catch (e) {
